...
print_key_help() {
  ...
  echo "WARNING: $key is missing in .env."
...
if [ ! -f .env ]; then
  env_file_exists=0
  echo "WARNING: .env is missing. Copy .env.example to .env and fill in your API keys."
fi

missing_env_key=0

if ! env_key_is_set "WORLD_LABS_API_KEY"; then
  print_key_help "WORLD_LABS_API_KEY" "3D environment generation (World Labs Marble 1.1)" "https://platform.worldlabs.ai/"
  missing_env_key=1
fi

if ! env_key_is_set "FAL_KEY"; then
  print_key_help "FAL_KEY" "3D models, SFX, and image editing via FAL" "https://fal.ai/"
  missing_env_key=1
fi

if [ "$missing_env_key" -eq 1 ]; then
  if [ "$env_file_exists" -eq 0 ]; then
    echo "   Tell the user: paste the key(s) here after visiting those URLs, and I can create .env from .env.example for you."
  else
...
if [ -d worlds ] && [ "$(ls worlds/ 2>/dev/null | wc -l | tr -d ' ')" -gt 0 ]; then
  ...
  echo "Casita projects available: $(echo "$WORLD_LIST" | wc -l | tr -d ' ') — $(echo "$WORLD_LIST" | tr '\n' ' ')"
else
  echo "No casita projects yet. Drop a photo in input/ and say 'casita-blast it' to begin."
fi
